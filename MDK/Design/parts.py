...
class Part:
    def __init__(self, name=None, type=None, info_file=None, layout_component_id=None, datasheet_link=None,layer_id=None):
        ...
        self.heatsink = False
        self.hsMaterial = None
        self.hsDimensions = [0, 0, 0, 0, 0, 0]    # heat sink x, y, z, basez, fins number, fins width 
        ...
        with open(self.info_file, 'r') as inputfile:
            ...
            for line in inputfile.readlines():
                ...
                if info[0] == "Name":
                    ...
                    if info[1] not in ['connector', 'MOSFET', 'Capacitor', 'Inductor', 'Diode', 'Terminal', 'Transformer', 'CPU', 'GPU', 'PSU']:  #== "component":
                        ...
                        print('type should be connector, MOSFET, Capacitor, Inductor, Diode, Terminal, Transformer', 'CPU', 'GPU', 'PSU')

                    if info[1] in ['MOSFET', 'Capacitor', 'Inductor', 'Diode', 'Terminal', 'Transformer', 'CPU', 'GPU', 'PSU']:  #== "component":
                            self.type = 1
                ...
                elif info[0] == 'Activation_Energy':
                    self.activationEnergy = float(info[1])
                if info[0] == "Pins":
                    pin_read = True
                ...
                if info[0] == "Heatsink":
                    ...
                    hs_read = True
                    continue
                if ("\t" in info[0]) and hs_read:
                    if info[0].strip("\t") == 'material':
                        self.hsMaterial = info[1]
                    if info[0].strip("\t") == 'dimensions':
                        self.hsDimensions[0] = float(info[1])
                        self.hsDimensions[1] = float(info[2])
                        self.hsDimensions[2] = float(info[3])
                        self.hsDimensions[3] = float(info[4])
                        self.hsDimensions[4] = float(info[5])
                        self.hsDimensions[5] = float(info[6])
                else:
                    hs_read = False
