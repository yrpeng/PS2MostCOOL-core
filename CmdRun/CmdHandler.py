...
# import OpenFoam API
from core.APIs.OpenFOAM.openFoamAPI import constructFoamCase
...
class CmdHandler: 
    def __init__(self,PSCore,debug=False):
        ...
        self.FOAM = {}  # for OpenFoam Parameters
        self.reliability = 0    # 0=no reliability , 1=simple reliabilit, 2=advanced reliability (for server)
    ...
    def load_macro_file(self, file):
        ...
        self.designInfo = {}  # Converter, Power Module, Server Design Info
        ...
        try:
            with open(file, 'r') as inputfile:
                ...
                for line in inputfile.readlines():
                    ...
                    if info[0] == "Reliability:":
                        self.reliability = int(info[1]) 
                    if info[0] =="New:":
                    ...
                    if(self.thermal_mode): # Get info for thermal setup
                     ...
                     # OpenFOAM Paremeters
                    #print(mode)
                    if info[0] == "Velocity:":  # OpenFOAM API 
                        self.FOAM['Velocity'] = float(info[1])
                    if info[0] == "Direction:":  # OpenFOAM API 
                        self.FOAM['Direction'] = info[1]
                    if info[0] == "TIM_Thickness:":  # OpenFOAM API 
                        self.FOAM['TIM_Thickness'] = float(info[1])
                    if info[0] == "TIM_Kappa:":  # OpenFOAM API 
                        self.FOAM['TIM_Kappa'] = float(info[1])
                    if info[0] == "Number_Core:":  # OpenFOAM API 
                        self.FOAM['Number_Core'] = float(info[1])
                    if info[0] == "Iteration:":  # OpenFOAM API 
                        self.FOAM['Iteration'] = float(info[1]) 
                    if info[0] == "Test_Temperature:":  # OpenFOAM API 
                        self.FOAM['Test_Temperature'] = float(info[1])
                    if info[0] == "Cabinet_Size:":  # OpenFOAM API
                        sizes = info[1].split(",")
                        sizes = [float(i) for i in sizes]
                        self.FOAM['cabinetSize'] = sizes
        ...
        if self.run_option == 0: # layout generation only, no need initial evaluation
            ...
            if self.designInfo['designType'] == 'Module':
    ...
    def serverInitialLayoutEvaluation(self, init = False):
        '''
        This functions evaluates the initial server layout given by the user.
        1. Convert the inital server layout script into PowerSynth Solution object
        2. Perform thermal evaluation using OpenFoam API
         '''
        #print(self.FOAM['Velocity'])
        ...
        for comp in self.structure_3D.layers[0].all_components:
            #print(comp.name)
            compsInfo[comp.name] = comp.activationEnergy
            if comp.heatsink:
                hsComps[comp.name] = [comp.hsMaterial, comp.hsDimensions]
        #print(hsComps)

        name ={}
        for l in self.structure_3D.layers[0].input_geometry:
            for item in range(len(l)):
                if l[item] in devices:
                    name[l[item]] = l[item + 1]

        for sol in PS_solutions:
            for feature in sol.features_list:
                if feature.name in devices:
                    ind = devices.index(feature.name)
                    feature.power = power[ind]
                    feature.name = name[feature.name]

        server = sol.features_list[1:3] + sol.features_list[4:]
        for comp in server:
            if comp.name == 'Bottom_Metal':
                comp.name = 'PCB'
                comp.material_name = 'FR4'
                comp.z = 0
                height = comp.height
            elif comp.name == 'Ceramic1':
                dims = self.FOAM['cabinetSize']
                dx = (dims[0] - comp.width)/2
                dy = (dims[1] - comp.length)/2
                #print(dims, comp.x, comp.y)
                comp.name = 'Air'
                comp.x = -dx
                comp.y = -dy
                comp.z = -5
                comp.width = comp.width + dx
                comp.length = comp.length + dy
                comp.height = dims[2]    # 95 for servers
                #sol.add_feature(name = 'Air', x=x, y=y, z=z, width=width, length=length , height=height, material_name='copper', power=0, h_val=0)
        ...
        for comp in server:
            for hs in hsComps:
                if  comp.name == hs:  #comp.name[1:3] == 'PU' or comp.name in ['MOSFET', 'rectifier']:
                    name = 'HS' + str(j)
                    material = hsComps[hs][0]
                    width = hsComps[hs][1][0]    #1.50 * comp.width 
                    length = hsComps[hs][1][1]    #1.50 * comp.length
                    height = hsComps[hs][1][2] 

                    hsParams = hsComps[hs][1][3:6]   
                        
                    x = comp.x - round((width - comp.width)/2, ndigits=3)
                    y = comp.y - round((length - comp.length)/2, ndigits=3)
                    z = comp.z + comp.height
                    sol.add_feature(name=name, x=x, y=y, z=z, width=width, length=length , height=height, material_name=material, power=hsParams, h_val=comp.name)
                    j+=1
        server = sol.features_list[1:3] + sol.features_list[4:]
        #for comp in server:
            #comp.printFeature()
        currentPath = os.getcwd()
        boundryConditions = [self.thermal_models_info, self.FOAM, compsInfo, self.reliability]
        print('Create OpenFOAM Case ')
        constructFoamCase(server, boundryConditions, currentPath, self.PSCore.FOAMDir, self.designInfo)
    ...
    def run_options(self):
        ...
        if self.run_option == 0: 
            ...
            if self.designInfo['designType'] == 'Server':
                self.serverInitialLayoutEvaluation()
            else:
